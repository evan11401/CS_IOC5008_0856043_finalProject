...
from train_classifier_xgboost import get_inputs
import pandas as pd
...
arg('--data_num', default="1")
arg('--test_fold', default="merged_test")
...
test_pred_folder = path.join(pred_folder, str(args.test_fold)+str(args.data_num))
...
extend_mask_test = path.join(pred_folder, 'merged_extend_test'+str(args.data_num))
...
if __name__ == '__main__':
    ...
    for it in range(num_split_iters):
        for it2 in range(folds_count):
            ...
            gbm_models.append(xgb.Booster(model_file=path.join(lgbm_models_folder, 'gbm_model_{0}_{1}.txt'.format(it, it2))))
    ...
    for f in tqdm(sorted(listdir(test_pred_folder))):
        if path.isfile(path.join(test_pred_folder, f)) and '.png' in f:
            ...
            #paramss.append((f, test_pred_folder, path.join(test_images_folder, img_id, 'images'), None))
    ...
    for sub_id in range(1):
        ...
        for f in tqdm(sorted(listdir(test_pred_folder))):
            if path.isfile(path.join(test_pred_folder, f)) and '.png' in f:
                ...
                for m in gbm_models:
                    if pred.shape[0] > 0:
                        #for xgboost
                        Dinp = xgb.DMatrix(inp)
                        pred += m.predict(Dinp)
                    for k in range(len(inputs2[im_idx])):
                        if pred2[k].shape[0] > 0:
                            Dinp = xgb.DMatrix(inputs2[im_idx][k])
                            pred2[k] += m.predict(Dinp)
                if pred.shape[0] > 0:
                    pred /= len(gbm_models)
        ...
        sub.to_csv(path.join(pred_folder, 'submission_{0}{1}.csv'.format(sub_id, args.data_num)), index=False)
