...
def get_inputs(filename, pred_folder, img_folder, truth_folder=None, extend_mask_folder=None):
    ...
    y_pred = measure.label(pred_msk, connectivity=2, background=0)
    ...
    pred_labels = measure.label(y_pred, connectivity=2, background=0)
    ...
    for i in range(len(pred_props)):
        ...
        for j in range(1, sep_count+1):
            ...
            lbls = measure.label(pred_reg2, connectivity=1, background=False)
            num_regs = lbls.max()
            if num_regs > 1:
                bst_j = j
                max_regs = num_regs
    ...
    if truth_folder is None:
        ...
        truth_labels = measure.label(truth_labels, connectivity=2, background=0)
...
if __name__ == '__main__':
    ...
    for it in range(num_split_iters):
        ...
        for train_idxs, test_idxs in kf.split(all_files):
            ...
            dtr, dte = xgb.DMatrix(train_inp,label=train_otp), xgb.DMatrix(test_inp,label=test_otp)
            params = {
                'max_depth': 6,
                'eta': 0.01,
